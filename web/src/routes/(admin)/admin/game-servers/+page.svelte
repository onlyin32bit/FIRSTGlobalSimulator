<script lang="ts">
	...
	async function create() {
		...
		try {
			...
			form = { origin: 'https://game.example.com' };
			await load();
		} catch (e) {
	...
	{#if createdKey}<div
		...
		>
			...
			<p class="mt-2 text-xs">
				Set <code>API_URL</code> and <code>GAME_SERVER_KEY</code> on the Rust host. No shared JWT secret is needed.
			</p>
		</div>{/if}
	<section class="rounded-xl border border-border bg-card p-5">
		<h2 class="font-semibold">Create a host</h2>
		<div class="mt-4 max-w-2xl">
			<Input placeholder="Public game-server URL (https://host)" bind:value={form.origin} />
		</div>
		<p class="mt-2 text-xs text-muted-foreground">The host reports its own user, match, and slot capacities when it connects.</p>
		<Button class="mt-4" disabled={submitting || !form.origin} onclick={create}
