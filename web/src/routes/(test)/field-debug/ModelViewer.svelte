<script lang="ts">
	...
	$effect(() => {
		const _ = showSkeleton;
		rebuildSkeletons();
	});
	$effect(() => {
		const _ = selectedUuid;
		updateSelection();
	});
	$effect(() => {
		if (!model) return;
				...
				for (const b of dynamicBodies) {
					...
					if (dist > 0.0001 && model) {
						...
						if (hits.length > 0) {
							const hit = hits[0];
							console.log(
								`[Ball ${b.id}] hit mesh "${hit.object.name}" at face index ${hit.faceIndex}`
							);
						}
					}
...
<div class="relative h-full min-h-[420px] overflow-hidden rounded-xl bg-slate-950">
	...
	<div
		class="pointer-events-none absolute bottom-3 left-3 rounded-md bg-black/55 px-2 py-1 text-[11px] text-slate-300"
	>
