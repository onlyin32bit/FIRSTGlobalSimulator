<script lang="ts">
	...
	function buildHierarchyRows(): Array<{ node: RuntimeNode; depth: number }> {
		...
		const matching = new SvelteSet<string>();
	...
	async function loadFile(file: File) {
		if (!file.name.toLowerCase().endsWith('.glb')) {
			error =
				'Choose a binary glTF (.glb). External .gltf resources cannot be resolved from one file.';
			return;
		}
		loading = true;
		error = '';
...
<svelte:head>
	...
	<meta name="description" content="Deep inspection workbench for FIRST Global field GLB assets." />
</svelte:head>
...
>
		...
		<div class="mx-auto flex max-w-[1900px] items-center gap-4 px-4 py-3">
			<div class="min-w-0 flex-1">
				<div class="flex items-center gap-2">
					<span
						class="rounded bg-cyan-400/10 px-2 py-0.5 font-mono text-[10px] font-bold tracking-widest text-cyan-300"
						>ASSET LAB</span
					>
	...
	{#if !inspection || !buffer}
		<main class="mx-auto flex min-h-[calc(100vh-72px)] max-w-5xl items-center px-4 py-10">
			...
			>
				<div
					class="mx-auto mb-5 grid h-20 w-20 place-items-center rounded-2xl border border-cyan-400/20 bg-cyan-400/10 text-4xl text-cyan-300"
				>
					⬡
				</div>
				...
				<p class="mx-auto mt-3 max-w-xl text-sm leading-6 text-slate-400">
					The file stays in your browser. Inspect the binary container, raw glTF document, runtime
					scene, GPU data, and rendered model without uploading it.
				</p>
				<div class="mt-7 flex flex-wrap justify-center gap-3">
					<button class="button-primary" onclick={() => input.click()}>Choose .glb file</button>
					<button class="button-secondary" onclick={loadBundledField}
						>Open bundled FGC 2026 field</button
					>
				</div>
				{#if loading}
				...
				{#if error}
					<div
						class="mx-auto mt-5 max-w-2xl rounded-lg border border-red-500/30 bg-red-500/10 p-3 text-left text-sm text-red-200"
					>
						{error}
					</div>
				{/if}
	...
	{:else}
		<main class="mx-auto max-w-[1900px] space-y-4 p-4">
			{#if dragging}
				<div
					class="pointer-events-none fixed inset-3 z-50 grid place-items-center rounded-2xl border-2 border-dashed border-cyan-300 bg-cyan-950/85 text-xl font-semibold text-cyan-100 backdrop-blur"
				>
					Drop to inspect another GLB
				</div>
			{/if}

			<section class="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-8">
				{#each [['Nodes', inspection.summary.nodes], ['Meshes', inspection.summary.meshes], ['Primitives', inspection.summary.primitives], ['Vertices', inspection.summary.vertices.toLocaleString()], ['Triangles', inspection.summary.triangles.toLocaleString()], ['Materials', inspection.summary.materials], ['Textures', inspection.summary.textures], ['Animations', inspection.summary.animations]] as metric (metric[0])}
				...
				<aside class="panel flex max-h-[680px] min-h-[420px] flex-col overflow-hidden">
					...
					<div class="border-b border-slate-800 p-2">
						<input
							...
							placeholder="Filter name, type, UUID…"
						/>
					</div>
						...
						<div class="space-y-4 p-3 text-xs">
							<section>
								...
								<dl class="property-grid">
									...
									<dt>UUID</dt>
									<dd class="break-all">{selectedNode.uuid}</dd>
									<dt>Visible</dt>
									<dd>{selectedNode.visible ? 'yes' : 'no'}</dd>
									<dt>Children</dt>
									<dd>{selectedNode.children.length}</dd>
									<dt>Render order</dt>
									<dd>{selectedNode.renderOrder}</dd>
								</dl>
							</section>
							<section>
								...
								<dl class="property-grid font-mono">
									<dt>Position</dt>
									<dd>{vector(selectedNode.position)}</dd>
									<dt>World</dt>
									<dd>{vector(selectedNode.worldPosition)}</dd>
									<dt>Rotation</dt>
									<dd>
									...
									<dt>Quaternion</dt>
									<dd>{vector(selectedNode.quaternion)}</dd>
									<dt>Scale</dt>
									<dd>{vector(selectedNode.scale)}</dd>
								</dl>
							</section>
							{#if selectedNode.bounds}
								<section>
									...
									<dl class="property-grid font-mono">
										<dt>Minimum</dt>
										<dd>{vector(selectedNode.bounds.min)}</dd>
										<dt>Maximum</dt>
										<dd>{vector(selectedNode.bounds.max)}</dd>
										<dt>Size</dt>
										<dd>{vector(selectedNode.bounds.size)}</dd>
									</dl>
								</section>
							{/if}
							{#if selectedNode.geometry}
								<section>
									...
									<dl class="property-grid">
										<dt>Name</dt>
										<dd>{selectedNode.geometry.name}</dd>
										<dt>Index count</dt>
										<dd>{selectedNode.geometry.indexCount.toLocaleString()}</dd>
										<dt>Groups</dt>
										<dd>{selectedNode.geometry.groups.length}</dd>
										<dt>Attributes</dt>
										<dd>{Object.keys(selectedNode.geometry.attributes).join(', ')}</dd>
									</dl>
									{#each Object.entries(selectedNode.geometry.attributes) as [name, attribute] (name)}
										<div
											class="mt-2 rounded border border-slate-800 bg-slate-950 p-2 font-mono text-[10px]"
										>
							...
							{#if selectedNode.materials?.length}
								<section>
									...
									{#each selectedNode.materials as material (material)}
										<div class="mb-2 rounded border border-slate-800 bg-slate-950 p-2">
											<strong>{material.name}</strong>
											<div class="mt-1 font-mono text-[10px] text-slate-400">
												{material.type} · opacity {material.opacity} · transparent {String(
													material.transparent
												)}
												{#if material.color}
													· color {material.color}{/if}
											</div>
										</div>
									{/each}
								</section>
			...
			<section class="panel">
				<div
					class="flex flex-wrap items-center gap-x-5 gap-y-2 border-b border-slate-800 p-3 text-xs"
				>
					<strong class="mr-1 text-slate-300">Viewer</strong>
					{#each [['Wireframe', wireframe, (value: boolean) => (wireframe = value)], ['Grid', showGrid, (value: boolean) => (showGrid = value)], ['Axes', showAxes, (value: boolean) => (showAxes = value)], ['Bounds', showBounds, (value: boolean) => (showBounds = value)], ['Normals', showNormals, (value: boolean) => (showNormals = value)], ['Skeleton', showSkeleton, (value: boolean) => (showSkeleton = value)]] as control (control[0])}
						<label class="flex cursor-pointer items-center gap-1.5 text-slate-400">
							<input
								type="checkbox"
								checked={control[1] as boolean}
								onchange={(event) =>
									(control[2] as (value: boolean) => void)(event.currentTarget.checked)}
							/>
							{control[0]}
						</label>
					{/each}
				...
				<div class="min-h-[360px] p-4">
					{#if activeTab === 'overview'}
						<div class="grid gap-4 lg:grid-cols-3">
							<article class="detail-card">
								...
								<dl class="property-grid">
									<dt>Generator</dt>
									<dd>{inspection.json.asset?.generator ?? '—'}</dd>
									<dt>Version</dt>
									<dd>{inspection.json.asset?.version ?? '—'}</dd>
									<dt>Min version</dt>
									<dd>{inspection.json.asset?.minVersion ?? '—'}</dd>
									<dt>Copyright</dt>
									<dd>{inspection.json.asset?.copyright ?? '—'}</dd>
									<dt>Default scene</dt>
									<dd>{inspection.json.scene ?? 0}</dd>
									<dt>Declared length</dt>
									<dd>{formatBytes(inspection.declaredLength)}</dd>
								</dl>
								{#if inspection.json.asset?.extras}
									<pre class="code-block mt-3">{JSON.stringify(
											inspection.json.asset.extras,
											null,
											2
										)}</pre>
								{/if}
							...
							<article class="detail-card">
								...
								{#each inspection.chunks as chunk (chunk.byteOffset)}
									<div
										class="mb-2 rounded border border-slate-800 bg-slate-950 p-2 font-mono text-xs"
									>
										<div class="flex justify-between">
											<strong class="text-cyan-300">{chunk.type}</strong><span
												>{formatBytes(chunk.byteLength)}</span
											>
										</div>
										<div class="mt-1 text-[10px] text-slate-500">
											header @{chunk.byteOffset} · data @{chunk.dataOffset} · {chunk.typeHex}
										</div>
									</div>
								{/each}
							</article>
							<article class="detail-card">
								...
								{#if runtime}
									<dl class="property-grid">
										<dt>Bounds size</dt>
										<dd>{vector(runtime.bounds.size, 3)}</dd>
										<dt>Bounds center</dt>
										<dd>{vector(runtime.bounds.center, 3)}</dd>
										<dt>Renderer</dt>
										<dd>{runtime.renderer.renderer}</dd>
										<dt>Vendor</dt>
										<dd>{runtime.renderer.vendor}</dd>
										<dt>API</dt>
										<dd>{runtime.renderer.webglVersion}</dd>
										<dt>Precision</dt>
										<dd>{runtime.renderer.precision}</dd>
										<dt>Max texture</dt>
										<dd>{runtime.renderer.maxTextureSize}px</dd>
										<dt>Max samples</dt>
										<dd>{runtime.renderer.maxSamples}</dd>
										<dt>Anisotropy</dt>
										<dd>{runtime.renderer.maxAnisotropy}×</dd>
									</dl>
								{:else}
							...
							<article class="detail-card lg:col-span-3">
								...
								<div class="table-wrap">
									<table>
										<thead
											><tr
												><th>#</th><th>Name</th><th>Root nodes</th><th>Extras</th><th>Extensions</th
												></tr
											></thead
										>
										<tbody>
											{#each inspection.json.scenes ?? [] as scene, index (index)}
												<tr
													><td>{index}</td><td>{scene.name ?? '—'}</td><td
														>{scene.nodes?.join(', ') ?? '—'}</td
													><td><code>{compact(scene.extras ?? {})}</code></td><td
														><code>{compact(scene.extensions ?? {})}</code></td
													></tr
												>
											{/each}
										</tbody>
					...
					{:else if activeTab === 'meshes'}
						<div class="space-y-3">
							{#each inspection.json.meshes ?? [] as mesh, meshIndex (meshIndex)}
								<details class="detail-card" open={meshIndex < 2}>
									<summary
										><strong>Mesh {meshIndex}: {mesh.name ?? '(unnamed)'}</strong>
										<span class="ml-2 text-xs text-slate-500"
											>{mesh.primitives?.length ?? 0} primitives · {mesh.weights?.length ?? 0} morph weights</span
										></summary
									>
									<div class="table-wrap mt-3">
										<table>
											<thead
												><tr
													><th>Primitive</th><th>Mode</th><th>Indices</th><th>Material</th><th
														>Attributes</th
													><th>Targets</th><th>Extensions</th></tr
												></thead
											>
											<tbody>
												{#each mesh.primitives ?? [] as primitive, primitiveIndex (primitiveIndex)}
													<tr>
					...
					{:else if activeTab === 'materials'}
						<div class="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
							{#each inspection.json.materials ?? [] as material, index (index)}
								<article class="detail-card">
									<div class="flex items-center justify-between">
										<h3>{index}: {material.name ?? '(unnamed)'}</h3>
										<span class="badge">{material.alphaMode ?? 'OPAQUE'}</span>
									</div>
									<dl class="property-grid">
										<dt>Double-sided</dt>
										<dd>{String(material.doubleSided ?? false)}</dd>
										<dt>Alpha cutoff</dt>
										<dd>{material.alphaCutoff ?? '—'}</dd>
										<dt>Base color</dt>
										<dd>
											<code
												>{compact(
										...
										<dt>Metallic</dt>
										<dd>{material.pbrMetallicRoughness?.metallicFactor ?? 1}</dd>
										<dt>Roughness</dt>
										<dd>{material.pbrMetallicRoughness?.roughnessFactor ?? 1}</dd>
										<dt>Emissive</dt>
										<dd><code>{compact(material.emissiveFactor ?? [0, 0, 0])}</code></dd>
									</dl>
						...
						<div class="grid gap-4 lg:grid-cols-2">
							<article class="detail-card">
								...
								<div class="table-wrap">
									<table>
										<thead
											><tr
												><th>#</th><th>Name</th><th>Source</th><th>Sampler</th><th>Extensions</th
												></tr
											></thead
										>
										<tbody
											>{#each inspection.json.textures ?? [] as texture, index (index)}<tr
													><td>{index}</td><td>{texture.name ?? '—'}</td><td
														>{texture.source ?? 'extension'}</td
													><td>{texture.sampler ?? 'default'}</td><td
														><code>{compact(texture.extensions ?? {})}</code></td
													></tr
												>{/each}</tbody
										>
									</table>
							...
							<article class="detail-card">
								...
								<div class="table-wrap">
									<table>
										<thead
											><tr><th>#</th><th>Name</th><th>MIME</th><th>bufferView / URI</th></tr></thead
										>
										<tbody
											>{#each inspection.json.images ?? [] as image, index (index)}<tr
													><td>{index}</td><td>{image.name ?? '—'}</td><td
														>{image.mimeType ?? 'inferred'}</td
													><td>{image.bufferView ?? image.uri ?? '—'}</td></tr
												>{/each}</tbody
										>
									</table>
								</div>
							...
							<article class="detail-card">
								...
								<div class="table-wrap max-h-[520px]">
									<table>
										<thead
											><tr
												><th>#</th><th>Name</th><th>Type</th><th>Component</th><th>Count</th><th
													>bufferView</th
												><th>Offset</th><th>Normalized</th><th>Min</th><th>Max</th><th>Sparse</th
												></tr
											></thead
										>
										<tbody
											>{#each inspection.json.accessors ?? [] as accessor, index (index)}<tr
													><td>{index}</td><td>{accessor.name ?? '—'}</td><td>{accessor.type}</td
													><td
														>{COMPONENT_TYPES[accessor.componentType] ?? accessor.componentType}</td
													><td>{accessor.count?.toLocaleString()}</td><td
														>{accessor.bufferView ?? '—'}</td
													><td>{accessor.byteOffset ?? 0}</td><td
														>{String(accessor.normalized ?? false)}</td
													><td><code>{compact(accessor.min ?? [])}</code></td><td
														><code>{compact(accessor.max ?? [])}</code></td
													><td>{accessor.sparse?.count ?? '—'}</td></tr
												>{/each}</tbody
										>
									</table>
								</div>
							...
							<article class="detail-card">
								...
								<div class="table-wrap max-h-[420px]">
									<table>
										<thead
											><tr
												><th>#</th><th>Name</th><th>Buffer</th><th>Offset</th><th>Length</th><th
													>Stride</th
												><th>Target</th><th>Extensions</th></tr
											></thead
										>
										<tbody
											>{#each inspection.json.bufferViews ?? [] as view, index (index)}<tr
													><td>{index}</td><td>{view.name ?? '—'}</td><td>{view.buffer ?? 0}</td><td
														>{view.byteOffset ?? 0}</td
													><td>{formatBytes(view.byteLength)}</td><td>{view.byteStride ?? '—'}</td
													><td>{BUFFER_TARGETS[view.target] ?? view.target ?? '—'}</td><td
														><code>{compact(view.extensions ?? {})}</code></td
													></tr
												>{/each}</tbody
										>
									</table>
					...
					{:else if activeTab === 'animations'}
						<div class="space-y-3">
							{#each inspection.json.animations ?? [] as animation, index (index)}
								<details class="detail-card" open>
									<summary
										><strong>{index}: {animation.name ?? '(unnamed)'}</strong>
										<span class="ml-2 text-xs text-slate-500"
											>{animation.channels?.length ?? 0} channels · {animation.samplers?.length ??
												0} samplers</span
										></summary
									>
							...
							<article class="detail-card">
								<h3>Skins</h3>
								<pre class="code-block">{JSON.stringify(inspection.json.skins ?? [], null, 2)}</pre>
							</article>
							<article class="detail-card">
								<h3>Cameras</h3>
								<pre class="code-block">{JSON.stringify(
										inspection.json.cameras ?? [],
										null,
										2
									)}</pre>
							</article>
						...
						<div class="grid gap-4 lg:grid-cols-2">
							<article class="detail-card">
								...
								<div class="mt-3 flex flex-wrap gap-2">
									{#each inspection.json.extensionsUsed ?? [] as extension (extension)}<span
											class="badge">{extension}</span
										>{/each}
								</div>
							</article>
							<article class="detail-card">
								...
								<div class="mt-3 flex flex-wrap gap-2">
									{#each inspection.json.extensionsRequired ?? [] as extension (extension)}<span
											class="badge !border-amber-500/30 !text-amber-300">{extension}</span
										>{/each}
								</div>
							</article>
							<article class="detail-card lg:col-span-2">
								<h3>Root extension payload</h3>
								<pre class="code-block">{JSON.stringify(
										inspection.json.extensions ?? {},
										null,
										2
									)}</pre>
							</article>
							<article class="detail-card lg:col-span-2">
								<h3>All extension occurrences</h3>
								<pre class="code-block">{JSON.stringify(extensionOccurrences(), null, 2)}</pre>
							</article>
						</div>
					{:else if activeTab === 'diagnostics'}
						<div class="space-y-2">
							{#each inspection.diagnostics as diagnostic (diagnostic)}
								<div
									class="diagnostic"
									class:error-diagnostic={diagnostic.level === 'error'}
									class:warning-diagnostic={diagnostic.level === 'warning'}
								>
							...
							{/each}
							<p class="pt-3 text-xs text-slate-500">
								These are structural and portability checks, not a substitute for Khronos glTF
								Validator conformance testing.
							</p>
						</div>
					{:else if activeTab === 'raw'}
						<div>
							<div class="mb-3 flex gap-2">
								...
								/>
								<button class="button-secondary" onclick={copyRawJson}
									>{copied ? 'Copied' : 'Copy JSON'}</button
								>
							</div>
							{#if rawFilter}
								<p class="mb-2 text-xs text-slate-500">{rawMatches.length} matching lines</p>
								<pre class="code-block max-h-[620px]">{rawMatches
										.map((match) => `${String(match.number).padStart(6)}  ${match.line}`)
										.join('\n')}</pre>
							{:else}
