<script lang="ts">
	...
	let parsedHpZones = $state<
		Record<string, { pos: [number, number, number]; bounds: HumanPlayerBounds }>
	>({
		red: {
	...
	onMount(() => {
		...
		Promise.all([loadJson(assetUrls.physics), loadJson(assetUrls.semantics)]).then(
			([physData, semData]) => {
				if (cancelled) return;
				...
				const computeZoneData = (zoneId: string) => {
					...
					const zoneQuaternion = new Quaternion().setFromEuler(new Euler(...zone.rotation));
					...
					for (let i = 0; i < zone.vertices.length; i += 3) {
						transformedVertex
							.set(zone.vertices[i], zone.vertices[i + 1], zone.vertices[i + 2])
	...
	function configureFieldVisual(scene: Object3D): Object3D {
		...
		scene.traverse((object) => {
			...
			// The imported field is static for the entire match. Avoid per-frame
			// matrix updates for every CAD mesh and let Three.js cull each part.
...
<T.Group>
	...
	{#if crosshairTextPos}
		<HTML position={crosshairTextPos} center pointerEvents="none">
			<div
				class="pointer-events-none rounded-md border border-white/20 bg-black/70 px-2.5 py-1 text-xs font-bold whitespace-nowrap text-white shadow-lg"
			>
